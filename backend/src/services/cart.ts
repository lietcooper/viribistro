...
// Use 2-decimal rounding consistent with Postgres Decimal(10, 2).
...
export async function addItem(
  ...
  const existing = readableItems(sessionId);
  const found = existing.find((i) => i.menuItemId === menuItemId);
  if (found) {
